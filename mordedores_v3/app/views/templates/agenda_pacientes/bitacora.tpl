                ...
                <td align="center">
                    {if $item.id_estado != 2}
                        <button type="button" data-toggle="tooltip" class="btn btn-xs btn-success" data-title="Aplicar Vacuna"
                                onclick="xModal.open('{$smarty.const.BASE_URI}/Agenda/aplicarVacuna/{$itm->id_agenda}','Aplicar Vacuna',60);">
                            <i class="fa fa-eyedropper"></i>
                        </button>
                    {/if}
                    <button type="button" data-toggle="tooltip" class="btn btn-xs btn-warning" data-title="Derivar"
                            onclick="xModal.open('{$smarty.const.BASE_URI}/Agenda/derivar/{$itm->id_agenda}','Derivar',60);">
                        <i class="fa fa-share-square-o"></i>
                    </button>
                    <button type="button" data-toggle="tooltip" class="btn btn-xs btn-info" data-title="Bitácora"
                            onclick="xModal.open('{$smarty.const.BASE_URI}/Agenda/bitacora/{$itm->id_agenda}','Bitácora Agenda',60);">
                        <i class="fa fa-eye"></i>
                    </button>
                </td>
            </tr>
