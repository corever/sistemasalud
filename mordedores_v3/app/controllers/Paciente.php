	...
	*/
	public function index() {
		...
		if($id_perfil == 1 || $id_perfil == 7 || $id_perfil == 8){
            ...
            $bo_filtros         = ($id_perfil == 3)?false:true; //Establecimiento Salud no ve filtros
            $bo_establecimiento = true;
		} else if($id_perfil == 12){
            // ADMINISTRATIVO = Ve los Sin Direccion de su Region
            /*
			if(!empty($id_oficina)){
            ...
            */
            $where      = array('id_region' => $id_region, 'id_oficina'=>$id_oficina);
            $bo_agrega  = 1;
			...
			//FISCALIZADOR
			$where              = array('id_fiscalizador'=>$id_usuario,'id_region'=>$id_region,'id_oficina'=>$id_oficina);
            $bandeja            = 'fiscalizador';
            $bo_fiscalizador    = true;
		} else if($id_perfil == 13){
			//COMUNAL
			$where              = array('id_region'=>$id_region,'id_oficina'=>$id_oficina,'comuna'=>$id_comuna);
            $bandeja            = 'comunal';
            $bo_fiscalizador    = true;
		} else{
			//TIC SEREMI
        ...
        if($arr){
            foreach ($arr as $key => &$expediente) {
            	/* CARGAR ALARMAS POR EXPEDIENTES */
                $where	= array(
                    "id_perfil"				=> $id_perfil,
        ...
        /* CARGAR GRILLA ALARMAS */
        $where	= array(
                "id_perfil"				=> $id_perfil,
                "id_establecimiento"	=> $id_establecimiento
            );
        $arrAlarmas    = $this->_DAOPacienteAlarma->getListaDetalle($where);
        foreach ((array) $arrAlarmas as &$alarma) {
            $alarma->paciente_telefonos_contacto	= array();
            $alarma->paciente_correos_contacto		= array();
            $datos_contacto							= $this->_DAOPacienteContacto->getByIdPaciente($alarma->id_paciente);
			
            if($datos_contacto){
                foreach ($datos_contacto as $contacto) {
                    $json_contacto = json_decode($contacto->json_dato_contacto);
                    if($contacto->id_tipo_contacto == 1){
                        $alarma->paciente_telefonos_contacto[] = $json_contacto->telefono_fijo;
                    }
                    if($contacto->id_tipo_contacto == 2){
                        $alarma->paciente_telefonos_contacto[] = $json_contacto->telefono_movil;
                    }
                    if($contacto->id_tipo_contacto == 4){
                        $alarma->paciente_correos_contacto[] = $json_contacto->gl_email;
                    }
                }
            }
        }
