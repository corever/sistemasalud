 ...
 * 
 * Descripcion       : Modelo para Tabla mor_adjunto_tipo
 *
 * Plataforma        : !PHP
 * 
 * Creacion          : 15/05/2018
...
class DAOAdjuntoTipo{

    protected $_tabla           = "mor_adjunto_tipo";
    ...
    public function getListaParaCombo(){
        $query  = " SELECT 
                        id_adjunto_tipo AS id, 
                        gl_nombre_tipo_adjunto AS nombre
                    FROM ".$this->_tabla."
                    WHERE bo_estado = 1";
        $result = $this->_conn->consultaArreglo($query);
