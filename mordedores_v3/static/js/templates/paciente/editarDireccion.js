$("#btn_buscar").on('click', function(e) {
    ...
    if (
        ($.trim($("#region").val()) == "0") &&
        ($.trim($("#id_oficina").val()) == "0") &&
        ($.trim($("#comuna").val()) == "0") &&
        ($.trim($("#id_fiscalizador").val()) == "0") &&
        ($.trim($("#establecimiento_salud").val()) == "0") &&
        ($.trim($("#folio_expediente").val()) == "") &&
        ($.trim($("#folio_mordedor").val()) == "") &&
        ($.trim($("#documento").val()) == "") &&
        ($.trim($("#microchip_mordedor").val()) == "")
    ) {
        msgError = "Debe ingresar al menos un filtro para continuar";
...
function verAlarma(boton) {
    ...
    $.ajax({
        ...
        data: parametros,
        type: "post",
        url: BASE_URI + 'index.php/EditarDireccion/cambiarEstadoAlarma',
        error: function(xhr, textStatus, errorThrown) {
