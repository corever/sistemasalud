...
var xModal = {
	...
	open : function(_item,_title,_size,id,_isIframe,_iframeHeight,_footer){
		...
		if(_size !== undefined){
			if(isNaN(_size)){
				...
				if(_size > 0){
					widthModal = 'width:'+_size + '%; max-width: initial;';
								...
								'<div class="modal-content" >'+
									'<div class="modal-header">'+
										'<h4 class="modal-title">'+titulo+'</h4>'+
											'<button type="button" class="close after-close" onclick="xModal.close();" aria-label="Close">'+
											'<span aria-hidden="true">×</span></button>'+
								  	'</div>'+
							      '<div class="modal-body" style="overflow:hidden;"><div id="progress">'+xModal.progress_bar+'</div> <div id="content-modal" style="display:none">'+
							      content+
							    '</div></div>'+
							    footer+
							    '</div>'+
		...
		$('body').append(innerModal);
		$(".modal").last().css('z-index',modalLast,'important')
		.on('hide.bs.modal',function(e){

			$(".modal").last().fadeOut(function(){
				$(this).remove();
	 ...
	 * @return {[type]} [description]
	 */
	close : function(modalIndex){this.success
        
        //Revisa si modal contenía un formulario con idform
        if($(".modal.fade.show").last().find("#idForm").val() !== undefined){
            //Base.cerrarModalRevisarSession();
        }
	...
	/**
	* Abrir un modal simple sin boton cerrar
	* @param  {[type]} _item         Puede ser un contenedor o una url a cargar
	* @param  {[type]} _title        Titulo del modal
	* @param  {[type]} _size         Ancho del modal, en porcentaje o lg,sm
	* @param  {[type]} _isIframe     Indica si el contenido a cargar en el modal corresponde a un iframe. Valor debe ser true
	...
	* @return {[type]}               [description]
	*/
   openSC : function(_item,_title,_size,id,_isIframe,_iframeHeight,_footer){
	   var window_width = window.innerWidth;
