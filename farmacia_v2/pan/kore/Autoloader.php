...
require_once __DIR__ . '/../utils/TemplateMenu.php';
        ...
        while ( $item = readdir ($app_dir)) {
            if ($item != "." and $item != "..") {
                if (is_dir('app' . DS . $item)) {
                    $module_dir = opendir('app' . DS . $item);
                    while ( $subitem = readdir ($module_dir) ) {
                        
                        foreach (self::$directories as $dir) {
                            if (is_dir('app' . DS . $item . DS . $subitem . DS . $dir) and is_file('app' . DS . $item . DS . $subitem . DS . $dir . DS . $className . '.php')) {
                                require_once 'app' . DS . $item . DS . $subitem . DS . $dir . DS . $className . '.php';
                            }
