...
class SessionPan{
    ...
    public static function getSessionAll($param)
    {
        $session_id = str_replace(' ', '', mb_strtolower(App::getName()));
    ...
    {   
        if (!isset($_SESSION[\Constantes::SESSION_BASE]['id_usuario'])) {
            header('Location: ' . \pan\uri\Uri::getHost());			
            die();
        }
        /*
        To do, revisar..aplica?
        if ( !isset($_SESSION[SESSION_BASE]['id_usuario'])) {
            header('Location: ' . BASE . 'Farmacia/Usuario/controllers/Login/index');			
            die();
        }
        return; 
        $session_id = str_replace(' ', '', mb_strtolower(App::getName()));
