<table class="table table-hover table-striped table-bordered dataTable no-footer" id="grilla">
	...
	<tbody>
        <?php foreach ($arrPerfiles as $key => $itm): ?>
		<tr>
			<td class="text-center"><?php echo $itm->gl_nombre_perfil ?></td>
			...
			<td class="text-center">
				<button type="button" class="btn btn-xs btn-success" title="<?php echo \Traduce::texto("Editar"); ?>" data-toggle="tooltip"
						onclick="xModal.open('<?php echo \Pan\Uri\Uri::getBaseUri(); ?>Farmacia/Mantenedor/Perfil/editarPerfil/<?php echo $itm->gl_token ?>','<?php echo \Traduce::texto('Editar Perfil'); ?>','xl');">
					<i class="fa fa-edit"></i></button>
				<!--<button type="button"  //VER SI PUEDE ELIMINAR
						onclick=""
						data-toggle="tooltip" title="Eliminar Perfil"
