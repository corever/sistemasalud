<table class="table table-hover table-striped table-bordered dataTable no-footer" id="grilla-roles-usuario">
	<thead>
		<tr>
			<th>Rol</th>
			<th>Asignación</th>
	...
	<tbody>
		<?php if(!empty($arrRolUsuario)): ?>
            <?php foreach ($arrRolUsuario as $key => $itm): ?>
                <tr>
                    ...
                    <td class="text-center">
                        <button type="button" class="btn btn-xs btn-danger" data-toggle="tooltip" title="Eliminar Rol"
                                onclick="MantenedorUsuario.eliminaRol(this,<?php echo $itm->mur_id; ?>)">
